...
func AuthMiddleware(next http.Handler) http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		if !ok {
			err = apperrors.Unauthorizated.Wrap(errors.New("name claim missing"), "invalid id token")
			apperrors.ErrorHandler(w, req, err)
			return
		}
		...
		if !ok {
			err = apperrors.Unauthorizated.Wrap(errors.New("name claim must be string"), "invalid id token")
			apperrors.ErrorHandler(w, req, err)
			return
		}

		// contextにユーザー名をセット
		req = common.SetUserName(req, nameStr)
